...
export default class Stack<T> {
    ...
    pop(): T | undefined {
        this.length = Math.max(0, this.length - 1); //bookkeeping, we must always stay at 0, can't have negative value as length
        if (this.length === 0) {
            const head = this.head; //save head
            this.head = undefined; //we don't need to have head anymore
            return head?.value;
        }
